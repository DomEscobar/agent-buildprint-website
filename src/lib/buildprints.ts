...
export const buildprints: Buildprint[] = [
  ...
  },
  {
    ...
    files: [
      ...
      { path: 'questions.md', purpose: 'closed alignment interview and default storyboard preset', required: true },
      { path: 'PLAN.md', purpose: 'phase index for numbered LLM execution rails', required: true },
      { path: 'plans/00-alignment.md', purpose: 'alignment phase: default preset or closed questions', required: true },
      { path: 'plans/01-domain-persistence.md', purpose: 'domain, persistence, deterministic IDs, and manifest assembly phase', required: true },
      { path: 'plans/02-agent-xml-pipeline.md', purpose: 'ScriptAgent/ProductionAgent deterministic XML pipeline phase', required: true },
      { path: 'plans/03-async-providers.md', purpose: 'job lifecycle, mock providers, retry/cancel, and no-network phase', required: true },
      { path: 'plans/04-workbench-ui.md', purpose: 'creative workbench UI, inspector, timeline, thumbnails, and debug drawer phase', required: true },
      { path: 'plans/05-qa-handover.md', purpose: 'QA, validation report, acceptance checks, and final chat handover phase', required: true },
      ...
      { path: 'LLM_AGENT_EXECUTION_GUIDE.md', purpose: 'LLM execution guide, context-rot safeguards, stage cards, and chat handover contract', required: true },
      { path: 'SPEC.md', purpose: canonicalFilePurposes['SPEC.md'], required: true },
      { path: 'CONTRACTS.md', purpose: canonicalFilePurposes['CONTRACTS.md'], required: true },
      { path: 'TEST_MATRIX.md', purpose: canonicalFilePurposes['TEST_MATRIX.md'], required: true },
      { path: 'WEBAPP_TARGET_SPEC.md', purpose: 'React/Vite target architecture, routes, pages, and state flows', required: true },
      { path: 'WORKBENCH_UX_SPEC.md', purpose: 'decision-complete creative workbench layout, inspector, timeline, and debug drawer spec', required: true },
      ...
      { path: 'VALIDATION_TEMPLATE.md', purpose: 'validation report template for choices, evidence, gaps, screenshots, and handover', required: true },
      { path: 'checks/acceptance.md', purpose: 'acceptance checklist for alignment, phases, visual QA, and final handover', required: true },
