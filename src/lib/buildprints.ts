...
export const buildprints: Buildprint[] = [
  ...
  },
  {
    ...
    stack: ['Repository census', 'Capability packs', 'Buildprint extraction', 'Verification rails'],
    ...
    checks: ['Minimal questions happen before discovery; deeper questions are deferred until evidence exists', 'Large repos produce candidates before final package unless full-system mode is selected', 'Generated discovery output is labeled scaffold unless a selected candidate is confirmed', 'Generated files contain no secret values', 'Claims are labeled OBSERVED/INFERRED/QUESTION/OUT_OF_SCOPE', 'Scope includes included/excluded paths', 'Output is source-independent: implementation agents do not need original source access', 'Capability packs define behavior, contracts, implementation order, and verification gates', 'EXECUTION_PROTOCOL.md, IMPLEMENTATION_PLAN.md, CURRENT_STATE.md, VERIFICATION.md, and manifest exist', 'Mocks/fixtures are not counted as product implementation', 'Included routes/services/providers/persistence/jobs/exports are real or explicitly excluded', 'Verification is derived from mapped flows and capability contracts', 'Reversal report and QA report state commands run, evidence, blockers, and known gaps', 'No validation results are invented', 'Capability proof ledger includes proof commands, artifacts, negative tests, runtime/browser evidence, and promotion blockers', 'Final chat handover states outcome, selected scope, evidence inspected, files generated, commands run, gaps, and next direction', 'No agb map CLI is required or advertised'],
    trustBadges: [
      ...
      { label: 'Proof-ledger gates', detail: 'Requires source evidence, product obligation, proof commands, artifacts, negative tests, and promotion blockers per capability.', tone: 'success' },
      { label: 'No fake MVPs', detail: 'Scope may be small, but included capabilities need proof or honest CONTRACT_SEAM_ONLY/BLOCKED_WITH_REASON status.', tone: 'warning' },
    ...
    copyPrompt: `${localPrompt('buildprint-mapper-os', 'Buildprint Mapper OS')} Use it to map this repo in the current agent session. Do not run agb map; that CLI does not exist. Ask only minimal safety/scope questions before discovery, then inspect the repo and create SYSTEM_MAP.md and BUILDPRINT_CANDIDATES.md before selected extraction unless I already picked a scope. Ask me to choose one candidate, then extract only that selected scope into buildprint-submission/ with BUILDPRINT.md, SPEC.md, CONTRACTS.md, PLAN.md, EXECUTION_PROTOCOL.md, IMPLEMENTATION_PLAN.md, CURRENT_STATE.md, VERIFICATION.md, capability packs when useful, REVERSAL_REPORT.md, QA_REPORT.md, manifest.json, and checks/acceptance.md. The package must be source-independent enough for a fresh coding agent to implement without original source access. Default to production-grade selected scope: smaller scope is OK, but do not create a lazy MVP, mock services as product behavior, placeholder routes, no-op controls, skeleton adapters, or in-memory-only persistence when durability is claimed. Do not copy secrets, do not invent validation results, and do not flatten the repo into one vague document. Finish with a chat handover containing outcome, selected scope, evidence inspected, files generated, commands run, known gaps, and recommended next direction.`,
  },
