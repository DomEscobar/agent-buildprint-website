...
export const buildprints: Buildprint[] = [
  ...
  },
  {
    ...
    checks: ['Codex dry-run generated a TypeScript billing proof', 'npm run build passed in dry-run proof', 'npm test passed: 6/6 lifecycle tests', 'Webhook signatures verified before state mutation', 'Premium access uses server-side subscription state', 'Portal requires authenticated customer', 'Failed/canceled/trialing states handled'],
    trustBadges: [
    ...
    copyPrompt: `${localPrompt('stripe-billing-extension', 'Stripe Billing Extension')} Implement billing with verified webhooks, server-side subscription state, entitlements, customer portal, billing UI, and lifecycle tests. Use env var names only; do not call real Stripe APIs in tests.`, 
  },
  {
