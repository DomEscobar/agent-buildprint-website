...
export const buildprints: Buildprint[] = [
  ...
  },
  {
    ...
    checks: ['Mapper executable packet validator passes', 'START_HERE.md routes to blueprint.yaml and active capability packet', '02-context/context-map.yaml blocks reading unrelated capability packets upfront', 'Each capability packet has capability.yaml, product-contract.md, implementation-workflow.md, source-evidence.md, and proof-contract.yaml', '09-evidence/evidence-ledger.jsonl is the claim-promotion authority', 'Claim status remains SELECTED_UNQUALIFIED until browser, provider, persistence, security, and clean-room proof rows pass'],
    trustBadges: [
    ...
    copyPrompt: `${localPrompt('portable-novel-storyboard-pipeline', 'Portable Novel-to-Storyboard Pipeline')} Treat the downloaded package as a Mapper OS executable packet. Read START_HERE.md, then blueprint.yaml, then 02-context/context-map.yaml. Load only the active capability packet named there, starting with 03-capabilities/project-workspace-auth/. Implement and prove that packet, append proof or blocker rows to 09-evidence/evidence-ledger.jsonl, then consult 03-capabilities/capability-index.yaml and continue one dependency-ready packet at a time. Do not read every capability packet upfront. Use deterministic no-network providers by default. Do not use Toonflow source as implementation input. Keep claims scoped to SELECTED_UNQUALIFIED until browser runtime, provider, persistence, security, and clean-room proof rows pass. Do not claim Toonflow clone, live provider parity, Electron parity, exact UI/canvas parity, or final stitched-video export parity.`,
  },
  {
    ...
    resultChecklist: ['SYSTEM_MAP.md explaining architecture, flows, data, integrations, side effects, tests, and unknowns', 'BUILDPRINT_CANDIDATES.md with 3-7 scoped reusable options', 'A selected package with BUILDPRINT.md, CAPABILITY_INDEX.md, CONTRACTS.md, TEAM_STACK.md, EXECUTION_PROTOCOL.md, IMPLEMENTATION_PLAN.md, CURRENT_STATE.md, VERIFICATION.md, manifest, and complete capability packs', 'UX_CONTRACT.md and DESIGN_QUALITY_BAR.md when the selected scope has user-facing UI', 'Capability proof ledger with evidence, commands, artifacts, negative tests, blockers, and honest SELECTED_UNQUALIFIED/BLOCKED_WITH_REASON statuses where proof is missing', 'Final handover explaining selected scope, files generated, commands run, known gaps, and next recommended action'],
    includes: ['Safety and secrets boundary', 'Machine-readable buildprint/phases/acceptance/claims rails', 'Minimal preflight questions', 'Repo census', 'SYSTEM_MAP.md', 'BUILDPRINT_CANDIDATES.md', 'Human scope decision gate', 'Dynamic post-discovery questions.md', 'Capability-oriented templates', 'TEAM_STACK.md team-pack routing', 'UX_CONTRACT.md for UI-bearing outputs', 'EXECUTION_PROTOCOL.md', 'IMPLEMENTATION_PLAN.md', 'CURRENT_STATE.md', 'VERIFICATION.md', 'Clean-room REVERSAL_REPORT.md', 'QA_REPORT.md', 'Single Buildprint extraction', 'Hierarchical System Buildprint extraction', 'Submission checklist', 'Final chat handover', 'Fixture review guidance'],
    ...
    files: [
      ...
      { path: 'CONTRACTS.md', purpose: canonicalFilePurposes['CONTRACTS.md'], required: true },
      { path: 'PLAN.md', purpose: canonicalFilePurposes['PLAN.md'], required: true },
      { path: 'EXECUTION_PROTOCOL.md', purpose: canonicalFilePurposes['EXECUTION_PROTOCOL.md'], required: true },
      { path: 'IMPLEMENTATION_PLAN.md', purpose: canonicalFilePurposes['IMPLEMENTATION_PLAN.md'], required: true },
      { path: 'VERIFICATION.md', purpose: canonicalFilePurposes['VERIFICATION.md'], required: true },
      ...
      { path: 'CURRENT_STATE.md', purpose: 'Buildprint package file', required: true },
      ...
      { path: 'evals/selected-output-fixtures/architecture-diagram-only/selected-buildprint/BUILDPRINT.md', purpose: 'negative selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/architecture-diagram-only/selected-buildprint/capabilities/01-diagram/CAPABILITY.md', purpose: 'negative selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/architecture-diagram-only/selected-buildprint/capabilities/01-diagram/IMPLEMENTATION.md', purpose: 'negative selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/architecture-diagram-only/selected-buildprint/capabilities/01-diagram/VERIFICATION.md', purpose: 'negative selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/architecture-diagram-only/selected-buildprint/CAPABILITY_INDEX.md', purpose: 'negative selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/architecture-diagram-only/selected-buildprint/CONTRACTS.md', purpose: 'negative selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/architecture-diagram-only/selected-buildprint/CURRENT_STATE.md', purpose: 'negative selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/architecture-diagram-only/selected-buildprint/EXECUTION_PROTOCOL.md', purpose: 'negative selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/architecture-diagram-only/selected-buildprint/IMPLEMENTATION_PLAN.md', purpose: 'negative selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/architecture-diagram-only/selected-buildprint/manifest.json', purpose: 'negative selected-output fixture manifest', required: true },
      { path: 'evals/selected-output-fixtures/architecture-diagram-only/selected-buildprint/PRE_IMPLEMENTATION_QUESTIONS.md', purpose: 'negative selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/architecture-diagram-only/selected-buildprint/TEAM_STACK.md', purpose: 'negative selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/architecture-diagram-only/selected-buildprint/VERIFICATION.md', purpose: 'negative selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/architecture-shell/selected-buildprint/BUILDPRINT.md', purpose: 'negative selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/architecture-shell/selected-buildprint/capabilities/01-shell/CAPABILITY.md', purpose: 'negative selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/architecture-shell/selected-buildprint/capabilities/01-shell/IMPLEMENTATION.md', purpose: 'negative selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/architecture-shell/selected-buildprint/capabilities/01-shell/VERIFICATION.md', purpose: 'negative selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/architecture-shell/selected-buildprint/CAPABILITY_INDEX.md', purpose: 'negative selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/architecture-shell/selected-buildprint/CONTRACTS.md', purpose: 'negative selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/architecture-shell/selected-buildprint/CURRENT_STATE.md', purpose: 'negative selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/architecture-shell/selected-buildprint/EXECUTION_PROTOCOL.md', purpose: 'negative selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/architecture-shell/selected-buildprint/IMPLEMENTATION_PLAN.md', purpose: 'negative selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/architecture-shell/selected-buildprint/manifest.json', purpose: 'negative selected-output fixture manifest', required: true },
      { path: 'evals/selected-output-fixtures/architecture-shell/selected-buildprint/PRE_IMPLEMENTATION_QUESTIONS.md', purpose: 'negative selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/architecture-shell/selected-buildprint/TEAM_STACK.md', purpose: 'negative selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/architecture-shell/selected-buildprint/VERIFICATION.md', purpose: 'negative selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/context-all-packs/selected-buildprint/BUILDPRINT.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/context-all-packs/selected-buildprint/capabilities/01-active/CAPABILITY.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/context-all-packs/selected-buildprint/capabilities/01-active/IMPLEMENTATION.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/context-all-packs/selected-buildprint/capabilities/01-active/VERIFICATION.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/context-all-packs/selected-buildprint/capabilities/01-ingestion-ontology/CAPABILITY.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/context-all-packs/selected-buildprint/capabilities/01-ingestion-ontology/IMPLEMENTATION.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/context-all-packs/selected-buildprint/capabilities/01-ingestion-ontology/VERIFICATION.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/context-all-packs/selected-buildprint/capabilities/02-unrelated/CAPABILITY.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/context-all-packs/selected-buildprint/capabilities/02-unrelated/IMPLEMENTATION.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/context-all-packs/selected-buildprint/capabilities/02-unrelated/VERIFICATION.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/context-all-packs/selected-buildprint/CAPABILITY_INDEX.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/context-all-packs/selected-buildprint/CONTEXT_PACKET.json', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/context-all-packs/selected-buildprint/CONTRACTS.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/context-all-packs/selected-buildprint/CURRENT_STATE.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/context-all-packs/selected-buildprint/DESIGN_QUALITY_BAR.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/context-all-packs/selected-buildprint/EXECUTION_PROTOCOL.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/context-all-packs/selected-buildprint/IMPLEMENTATION_PLAN.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/context-all-packs/selected-buildprint/manifest.json', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/context-all-packs/selected-buildprint/PRE_IMPLEMENTATION_QUESTIONS.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/context-all-packs/selected-buildprint/SOURCE_SURFACE_COVERAGE.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/context-all-packs/selected-buildprint/TEAM_STACK.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/context-all-packs/selected-buildprint/UX_CONTRACT.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/context-all-packs/selected-buildprint/VERIFICATION.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/executable-packet-good/selected-buildprint/00-intent/mission.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/executable-packet-good/selected-buildprint/00-intent/product-obligations.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/executable-packet-good/selected-buildprint/00-intent/source-surface-map.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/executable-packet-good/selected-buildprint/01-operating-model/autonomy-levels.yaml', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/executable-packet-good/selected-buildprint/01-operating-model/human-approval-policy.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/executable-packet-good/selected-buildprint/01-operating-model/stop-rules.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/executable-packet-good/selected-buildprint/01-operating-model/workflow-vs-agentic.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/executable-packet-good/selected-buildprint/02-context/context-map.yaml', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/executable-packet-good/selected-buildprint/02-context/read-order.yaml', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/executable-packet-good/selected-buildprint/02-context/source-evidence-index.yaml', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/executable-packet-good/selected-buildprint/03-capabilities/capability-index.yaml', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/executable-packet-good/selected-buildprint/03-capabilities/ingest-record/capability.yaml', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/executable-packet-good/selected-buildprint/03-capabilities/ingest-record/implementation-workflow.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/executable-packet-good/selected-buildprint/03-capabilities/ingest-record/product-contract.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/executable-packet-good/selected-buildprint/03-capabilities/ingest-record/proof-contract.yaml', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/executable-packet-good/selected-buildprint/03-capabilities/ingest-record/source-evidence.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/executable-packet-good/selected-buildprint/04-interfaces/api-contracts.yaml', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/executable-packet-good/selected-buildprint/04-interfaces/provider-contracts.yaml', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/executable-packet-good/selected-buildprint/04-interfaces/schemas/.gitkeep', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/executable-packet-good/selected-buildprint/04-interfaces/tool-contracts.yaml', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/executable-packet-good/selected-buildprint/05-state-runtime/persistence.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/executable-packet-good/selected-buildprint/05-state-runtime/runtime-topology.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/executable-packet-good/selected-buildprint/05-state-runtime/state-model.yaml', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/executable-packet-good/selected-buildprint/06-safety/destructive-actions.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/executable-packet-good/selected-buildprint/06-safety/secrets-policy.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/executable-packet-good/selected-buildprint/06-safety/threat-model.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/executable-packet-good/selected-buildprint/07-execution/implementation-plan.yaml', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/executable-packet-good/selected-buildprint/07-execution/phases/00-ingest.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/executable-packet-good/selected-buildprint/08-evaluation/acceptance.yaml', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/executable-packet-good/selected-buildprint/08-evaluation/quality-rubric.yaml', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/executable-packet-good/selected-buildprint/08-evaluation/test-matrix.yaml', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/executable-packet-good/selected-buildprint/09-evidence/evidence-ledger.jsonl', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/executable-packet-good/selected-buildprint/09-evidence/unresolved-blockers.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/executable-packet-good/selected-buildprint/blueprint.yaml', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/executable-packet-good/selected-buildprint/BUILDPRINT.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/executable-packet-good/selected-buildprint/generated/agent-prompt.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/executable-packet-good/selected-buildprint/generated/current-buildprint-compat/BUILDPRINT.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/executable-packet-good/selected-buildprint/generated/current-buildprint-compat/CAPABILITY_INDEX.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/executable-packet-good/selected-buildprint/generated/current-buildprint-compat/CURRENT_STATE.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/executable-packet-good/selected-buildprint/generated/current-buildprint-compat/VERIFICATION.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/executable-packet-good/selected-buildprint/START_HERE.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/microfish-bad-shape/selected-buildprint/BUILDPRINT.md', purpose: 'negative selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/microfish-bad-shape/selected-buildprint/capabilities/ingestion-ontology/CAPABILITY.md', purpose: 'negative incomplete capability fixture', required: true },
      { path: 'evals/selected-output-fixtures/microfish-bad-shape/selected-buildprint/HANDOFF.md', purpose: 'negative selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/microfish-bad-shape/selected-buildprint/HANDOVER.md', purpose: 'negative selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/microfish-bad-shape/selected-buildprint/manifest.json', purpose: 'negative selected-output fixture manifest', required: true },
      { path: 'evals/selected-output-fixtures/microfish-bad-shape/selected-buildprint/VERFICATION.md', purpose: 'negative typo fixture', required: true },
      { path: 'evals/selected-output-fixtures/microfish-good-shape/selected-buildprint/BUILDPRINT.md', purpose: 'passing selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/microfish-good-shape/selected-buildprint/capabilities/01-ingestion-ontology/CAPABILITY.md', purpose: 'passing selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/microfish-good-shape/selected-buildprint/capabilities/01-ingestion-ontology/IMPLEMENTATION.md', purpose: 'passing selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/microfish-good-shape/selected-buildprint/capabilities/01-ingestion-ontology/VERIFICATION.md', purpose: 'passing selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/microfish-good-shape/selected-buildprint/CAPABILITY_INDEX.md', purpose: 'passing selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/microfish-good-shape/selected-buildprint/CONTEXT_PACKET.json', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/microfish-good-shape/selected-buildprint/CONTRACTS.md', purpose: 'passing selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/microfish-good-shape/selected-buildprint/CURRENT_STATE.md', purpose: 'passing selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/microfish-good-shape/selected-buildprint/DESIGN_QUALITY_BAR.md', purpose: 'passing selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/microfish-good-shape/selected-buildprint/EXECUTION_PROTOCOL.md', purpose: 'passing selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/microfish-good-shape/selected-buildprint/IMPLEMENTATION_PLAN.md', purpose: 'passing selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/microfish-good-shape/selected-buildprint/manifest.json', purpose: 'passing selected-output fixture manifest', required: true },
      { path: 'evals/selected-output-fixtures/microfish-good-shape/selected-buildprint/PRE_IMPLEMENTATION_QUESTIONS.md', purpose: 'passing selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/microfish-good-shape/selected-buildprint/SOURCE_SURFACE_COVERAGE.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/microfish-good-shape/selected-buildprint/TEAM_STACK.md', purpose: 'passing selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/microfish-good-shape/selected-buildprint/UX_CONTRACT.md', purpose: 'passing selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/microfish-good-shape/selected-buildprint/VERIFICATION.md', purpose: 'passing selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/missing-team-stack/selected-buildprint/BUILDPRINT.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/missing-team-stack/selected-buildprint/capabilities/01-ingestion-ontology/CAPABILITY.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/missing-team-stack/selected-buildprint/capabilities/01-ingestion-ontology/IMPLEMENTATION.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/missing-team-stack/selected-buildprint/capabilities/01-ingestion-ontology/VERIFICATION.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/missing-team-stack/selected-buildprint/CAPABILITY_INDEX.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/missing-team-stack/selected-buildprint/CONTRACTS.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/missing-team-stack/selected-buildprint/CURRENT_STATE.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/missing-team-stack/selected-buildprint/DESIGN_QUALITY_BAR.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/missing-team-stack/selected-buildprint/EXECUTION_PROTOCOL.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/missing-team-stack/selected-buildprint/IMPLEMENTATION_PLAN.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/missing-team-stack/selected-buildprint/manifest.json', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/missing-team-stack/selected-buildprint/PRE_IMPLEMENTATION_QUESTIONS.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/missing-team-stack/selected-buildprint/UX_CONTRACT.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/missing-team-stack/selected-buildprint/VERIFICATION.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/old-read-order/selected-buildprint/BUILDPRINT.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/old-read-order/selected-buildprint/capabilities/01-ingestion-ontology/CAPABILITY.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/old-read-order/selected-buildprint/capabilities/01-ingestion-ontology/IMPLEMENTATION.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/old-read-order/selected-buildprint/capabilities/01-ingestion-ontology/VERIFICATION.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/old-read-order/selected-buildprint/CAPABILITY_INDEX.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/old-read-order/selected-buildprint/CONTEXT_PACKET.json', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/old-read-order/selected-buildprint/CONTRACTS.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/old-read-order/selected-buildprint/CURRENT_STATE.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/old-read-order/selected-buildprint/DESIGN_QUALITY_BAR.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/old-read-order/selected-buildprint/EXECUTION_PROTOCOL.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/old-read-order/selected-buildprint/IMPLEMENTATION_PLAN.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/old-read-order/selected-buildprint/manifest.json', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/old-read-order/selected-buildprint/PRE_IMPLEMENTATION_QUESTIONS.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/old-read-order/selected-buildprint/SOURCE_SURFACE_COVERAGE.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/old-read-order/selected-buildprint/TEAM_STACK.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/old-read-order/selected-buildprint/UX_CONTRACT.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/old-read-order/selected-buildprint/VERIFICATION.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/pretty-fake-ui/selected-buildprint/BUILDPRINT.md', purpose: 'negative selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/pretty-fake-ui/selected-buildprint/capabilities/01-pretty-shell/CAPABILITY.md', purpose: 'negative selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/pretty-fake-ui/selected-buildprint/capabilities/01-pretty-shell/IMPLEMENTATION.md', purpose: 'negative selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/pretty-fake-ui/selected-buildprint/capabilities/01-pretty-shell/VERIFICATION.md', purpose: 'negative selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/pretty-fake-ui/selected-buildprint/CAPABILITY_INDEX.md', purpose: 'negative selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/pretty-fake-ui/selected-buildprint/CONTRACTS.md', purpose: 'negative selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/pretty-fake-ui/selected-buildprint/CURRENT_STATE.md', purpose: 'negative selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/pretty-fake-ui/selected-buildprint/DESIGN_QUALITY_BAR.md', purpose: 'negative selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/pretty-fake-ui/selected-buildprint/EXECUTION_PROTOCOL.md', purpose: 'negative selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/pretty-fake-ui/selected-buildprint/IMPLEMENTATION_PLAN.md', purpose: 'negative selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/pretty-fake-ui/selected-buildprint/manifest.json', purpose: 'negative selected-output fixture manifest', required: true },
      { path: 'evals/selected-output-fixtures/pretty-fake-ui/selected-buildprint/PRE_IMPLEMENTATION_QUESTIONS.md', purpose: 'negative selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/pretty-fake-ui/selected-buildprint/TEAM_STACK.md', purpose: 'negative selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/pretty-fake-ui/selected-buildprint/UX_CONTRACT.md', purpose: 'negative selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/pretty-fake-ui/selected-buildprint/VERIFICATION.md', purpose: 'negative selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/ui-missing-ux/selected-buildprint/BUILDPRINT.md', purpose: 'negative selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/ui-missing-ux/selected-buildprint/capabilities/01-dashboard/CAPABILITY.md', purpose: 'negative selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/ui-missing-ux/selected-buildprint/capabilities/01-dashboard/IMPLEMENTATION.md', purpose: 'negative selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/ui-missing-ux/selected-buildprint/capabilities/01-dashboard/VERIFICATION.md', purpose: 'negative selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/ui-missing-ux/selected-buildprint/CAPABILITY_INDEX.md', purpose: 'negative selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/ui-missing-ux/selected-buildprint/CONTRACTS.md', purpose: 'negative selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/ui-missing-ux/selected-buildprint/CURRENT_STATE.md', purpose: 'negative selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/ui-missing-ux/selected-buildprint/EXECUTION_PROTOCOL.md', purpose: 'negative selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/ui-missing-ux/selected-buildprint/IMPLEMENTATION_PLAN.md', purpose: 'negative selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/ui-missing-ux/selected-buildprint/manifest.json', purpose: 'negative selected-output fixture manifest', required: true },
      { path: 'evals/selected-output-fixtures/ui-missing-ux/selected-buildprint/PRE_IMPLEMENTATION_QUESTIONS.md', purpose: 'negative selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/ui-missing-ux/selected-buildprint/TEAM_STACK.md', purpose: 'negative selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/ui-missing-ux/selected-buildprint/VERIFICATION.md', purpose: 'negative selected-output fixture', required: true },
      { path: 'evals/selected-output-fixtures/v2-claimed-proof-without-evidence/selected-buildprint/00-intent/product-obligations.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/v2-claimed-proof-without-evidence/selected-buildprint/00-intent/source-surface-map.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/v2-claimed-proof-without-evidence/selected-buildprint/03-capabilities/bad/capability.yaml', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/v2-claimed-proof-without-evidence/selected-buildprint/03-capabilities/bad/proof-contract.yaml', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/v2-claimed-proof-without-evidence/selected-buildprint/03-capabilities/capability-index.yaml', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/v2-claimed-proof-without-evidence/selected-buildprint/09-evidence/evidence-ledger.jsonl', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/v2-claimed-proof-without-evidence/selected-buildprint/blueprint.yaml', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/v2-claimed-proof-without-evidence/selected-buildprint/BUILDPRINT.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/v2-claimed-proof-without-evidence/selected-buildprint/generated/agent-prompt.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/v2-claimed-proof-without-evidence/selected-buildprint/START_HERE.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/v2-missing-obligation-routing/selected-buildprint/00-intent/product-obligations.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/v2-missing-obligation-routing/selected-buildprint/00-intent/source-surface-map.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/v2-missing-obligation-routing/selected-buildprint/02-context/source-evidence-index.yaml', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/v2-missing-obligation-routing/selected-buildprint/03-capabilities/bad/capability.yaml', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/v2-missing-obligation-routing/selected-buildprint/03-capabilities/bad/proof-contract.yaml', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/v2-missing-obligation-routing/selected-buildprint/03-capabilities/capability-index.yaml', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/v2-missing-obligation-routing/selected-buildprint/09-evidence/evidence-ledger.jsonl', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/v2-missing-obligation-routing/selected-buildprint/blueprint.yaml', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/v2-missing-obligation-routing/selected-buildprint/BUILDPRINT.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/v2-missing-obligation-routing/selected-buildprint/generated/agent-prompt.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/v2-missing-obligation-routing/selected-buildprint/START_HERE.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/v2-missing-proof-contract/selected-buildprint/00-intent/product-obligations.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/v2-missing-proof-contract/selected-buildprint/00-intent/source-surface-map.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/v2-missing-proof-contract/selected-buildprint/03-capabilities/bad/capability.yaml', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/v2-missing-proof-contract/selected-buildprint/03-capabilities/capability-index.yaml', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/v2-missing-proof-contract/selected-buildprint/09-evidence/evidence-ledger.jsonl', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/v2-missing-proof-contract/selected-buildprint/blueprint.yaml', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/v2-missing-proof-contract/selected-buildprint/BUILDPRINT.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/v2-missing-proof-contract/selected-buildprint/generated/agent-prompt.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/v2-missing-proof-contract/selected-buildprint/START_HERE.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/v2-stale-generated-prompt/selected-buildprint/00-intent/product-obligations.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/v2-stale-generated-prompt/selected-buildprint/00-intent/source-surface-map.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/v2-stale-generated-prompt/selected-buildprint/03-capabilities/bad/capability.yaml', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/v2-stale-generated-prompt/selected-buildprint/03-capabilities/bad/proof-contract.yaml', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/v2-stale-generated-prompt/selected-buildprint/03-capabilities/capability-index.yaml', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/v2-stale-generated-prompt/selected-buildprint/09-evidence/evidence-ledger.jsonl', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/v2-stale-generated-prompt/selected-buildprint/blueprint.yaml', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/v2-stale-generated-prompt/selected-buildprint/BUILDPRINT.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/v2-stale-generated-prompt/selected-buildprint/generated/agent-prompt.md', purpose: 'evaluation harness artifact', required: true },
      { path: 'evals/selected-output-fixtures/v2-stale-generated-prompt/selected-buildprint/START_HERE.md', purpose: 'evaluation harness artifact', required: true },
      ...
      { path: 'policies/quality.md', purpose: 'policy artifact', required: false },
      ...
      { path: 'templates/BUILDPRINT.md', purpose: 'Buildprint package file', required: true },
      { path: 'templates/capabilities/_template/CAPABILITY.md', purpose: 'Buildprint package file', required: true },
      { path: 'templates/capabilities/_template/CONTRACTS.md', purpose: 'Buildprint package file', required: true },
      { path: 'templates/capabilities/_template/IMPLEMENTATION.md', purpose: 'Buildprint package file', required: true },
      { path: 'templates/capabilities/_template/VERIFICATION.md', purpose: 'Buildprint package file', required: true },
      { path: 'templates/CAPABILITY_INDEX.md', purpose: 'Buildprint package file', required: true },
      { path: 'templates/CONTEXT_PACKET.json', purpose: 'machine-readable mirror', required: true },
      { path: 'templates/CONTRACTS.md', purpose: 'Buildprint package file', required: true },
      ...
      { path: 'templates/DESIGN_QUALITY_BAR.md', purpose: 'Buildprint package file', required: true },
      { path: 'templates/executable-packet/00-intent/source-surface-map.md', purpose: 'Buildprint package file', required: true },
      { path: 'templates/executable-packet/02-context/source-evidence-index.yaml', purpose: 'machine-readable mirror', required: true },
      { path: 'templates/executable-packet/03-capabilities/_template/capability.yaml', purpose: 'machine-readable mirror', required: true },
      { path: 'templates/executable-packet/03-capabilities/_template/implementation-workflow.md', purpose: 'Buildprint package file', required: true },
      { path: 'templates/executable-packet/03-capabilities/_template/product-contract.md', purpose: 'Buildprint package file', required: true },
      { path: 'templates/executable-packet/03-capabilities/_template/proof-contract.yaml', purpose: 'machine-readable mirror', required: true },
      { path: 'templates/executable-packet/03-capabilities/_template/source-evidence.md', purpose: 'Buildprint package file', required: true },
      { path: 'templates/executable-packet/03-capabilities/capability-index.yaml', purpose: 'machine-readable mirror', required: true },
      { path: 'templates/executable-packet/08-evaluation/acceptance.yaml', purpose: 'machine-readable mirror', required: true },
      { path: 'templates/executable-packet/09-evidence/evidence-ledger.jsonl', purpose: 'Buildprint package file', required: true },
      { path: 'templates/executable-packet/blueprint.yaml', purpose: 'machine-readable mirror', required: true },
      { path: 'templates/executable-packet/BUILDPRINT.md', purpose: 'Buildprint package file', required: true },
      { path: 'templates/executable-packet/generated/agent-prompt.md', purpose: 'Buildprint package file', required: true },
      { path: 'templates/executable-packet/START_HERE.md', purpose: 'Buildprint package file', required: true },
      { path: 'templates/EXECUTION_PROTOCOL.md', purpose: 'Buildprint package file', required: true },
      ...
      { path: 'templates/SECURITY.md', purpose: 'Buildprint package file', required: true },
      { path: 'templates/SOURCE_SURFACE_COVERAGE.md', purpose: 'Buildprint package file', required: true },
      { path: 'templates/TEAM_STACK.md', purpose: 'selected-output team-pack quality gate template', required: true },
      { path: 'templates/teams/data-persistence.md', purpose: 'team-pack rule template for durable state and lifecycle', required: true },
      { path: 'templates/teams/integration-runtime.md', purpose: 'Buildprint package file', required: true },
      { path: 'templates/teams/product-architect.md', purpose: 'team-pack rule template for topology and first real vertical slice', required: true },
      { path: 'templates/teams/security-boundary.md', purpose: 'team-pack rule template for sensitive operations and secrets', required: true },
      { path: 'templates/teams/test-and-verification.md', purpose: 'team-pack rule template for proof ledgers and no-fake checks', required: true },
      { path: 'templates/teams/ux-ui-craft.md', purpose: 'team-pack rule template for UI workflow quality and browser proof', required: true },
      { path: 'templates/UX_CONTRACT.md', purpose: 'Buildprint package file', required: true },
      { path: 'templates/VERIFICATION.md', purpose: 'Buildprint package file', required: true },
    ],
    checks: ['Minimal questions happen before discovery; deeper questions are deferred until evidence exists', 'Large repos produce candidates before final package unless full-system mode is selected', 'Generated discovery output is labeled scaffold unless a selected candidate is confirmed', 'Generated files contain no secret values', 'Claims are labeled OBSERVED/INFERRED/QUESTION/OUT_OF_SCOPE', 'Scope includes included/excluded paths', 'Output is source-independent: implementation agents do not need original source access', 'Capability packs define behavior, contracts, implementation order, and verification gates', 'TEAM_STACK.md is generated for selected outputs and required teams are inferred from product signals', 'UX_CONTRACT.md and DESIGN_QUALITY_BAR.md are required for UI-bearing selected outputs', 'EXECUTION_PROTOCOL.md, IMPLEMENTATION_PLAN.md, CURRENT_STATE.md, VERIFICATION.md, TEAM_STACK.md, and manifest exist', 'Mocks/fixtures are not counted as product implementation', 'Included routes/services/providers/persistence/jobs/exports are real or explicitly excluded', 'Verification is derived from mapped flows and capability contracts', 'Capability proof ledger includes proof commands, artifacts, negative tests, runtime/browser evidence, and promotion blockers', 'Reversal report and QA report state commands run, evidence, blockers, and known gaps', 'No validation results are invented', 'Final chat handover states outcome, selected scope, evidence inspected, files generated, commands run, gaps, and next direction', 'No agb map CLI is required or advertised'],
    trustBadges: [
...
export function packageManifest(bp: Buildprint) {
  ...
  const readOrder = (isExecutablePacket ? ['BUILDPRINT.md', 'START_HERE.md', 'blueprint.yaml'] : ['BUILDPRINT.md']).filter(hasFile);
